...
class GP_model:
    ...
    def negative_loglikelihood(self, hyper, X, Y):
        ...
        K = K + (sn2 + 1e-7) * np.eye(n_point)  # (nxn) covariance matrix
    ...
    # --- GP inference --- #
    ########################
    def GP_predictor(self, kernel='Matern32'):# , X,Y):
...
class ITR_GP_RTO:
    ...
    def __init__(self, obj_model, obj_system, cons_model, cons_system, x0,
                 ...
                 model=None,kernel='RBF', Use_GP_for_model=True):
        '''
    ...
    def construct_opt_ca_with_GPception(self, d_init, uk, GP_obj, GP_con, GP_obj_model, GP_con_model):
        ...
        lbg = []
        ubg = []
        U = MX.sym('u_' + str(0), np.shape(d_init)[0])
        w += [U]
        lbw.extend([0] * np.shape(d_init)[0])
        ubw.extend([1] * np.shape(d_init)[0])
        w0.extend(u_init)
        #     x1 = f(x,u)* (not(model.empty)).real
        #     x = MX.sym('x_'+str(i+1),model.nd)
        #     X += [x1]
        ...
        #     gg += [model.bio_con2_ca_f(x1)]
        # Sum_slack = 0
        for i in range(len(self.cons_system)):#self.model.nk*self.model.nu):
